...
export interface BillingSettingsContentProps {
  /** When set, rendered above billing content with page spacing (standalone billing page). */
  header?: ReactNode;
  /** Applied to the outer wrapper when present; use with `header` for max-width, etc. */
...
export function BillingSettingsContent({
  ...
  if (header != null) {
    return (
      <div className={cn("space-y-8", wrapperClassName)}>
        {header}
        {inner}
      </div>
    );
